		...
		public function init()
		{
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				if (!isset($_REQUEST['_wpnonce'])) {
					die('<div class="error settings-error notice">WP nonce not set!</div>');
				}

				if (!wp_verify_nonce($_REQUEST['_wpnonce'], 'pricerunner_form')) {
					die('<div class="error settings-error notice">WP nonce not valid! Please go back and refresh the page to try again.</div>');
				}
			}
